...
\section{V3 Cached Evidence Audit}

The v3 paper adds a cached audit layer over the full controlled run, the sparse
learned-ensemble run, the sweep, and the closed-loop sanity check. The script
\path{experiments/v3_cached_evidence.py} reads \VThreeControlledRows{}
controlled rollout rows, \VThreeTraceRows{} CEM trace rows,
\VThreeSweepRows{} sweep rows, \VThreeRepairGainRows{} repair-gain rows, and
\VThreeLearnedRows{} learned-ensemble rollout rows. It writes compact CSV
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\linewidth]{figures/v3/v3_controlled_regret_rank.pdf}
\caption{V3 controlled-run planner ranking. Vanilla CEM has substantially
...
mean regret \VThreeCEMRegret{}, while the static-proposal baseline has
\VThreeStaticRegret{} and the equal-budget static baseline has
\VThreeEqualStaticRegret{}. The adaptive refit therefore costs
\VThreeCEMMinusStatic{} regret points relative to one-shot static proposals in
the controlled pocket world. The best repair has regret
\VThreeBestRepairRegret{}, a \VThreeBestRepairGain{} point reduction from
vanilla \cem{}.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{figures/v3/v3_cem_trace_progression.pdf}
...
Figure~\ref{fig:v3-trace} targets the mechanism rather than only final regret.
Across vanilla \cem{} traces, selected tail gap rises from
\VThreeTraceTailGapStart{} to \VThreeTraceTailGapEnd{} and elite pocket mass
rises from \VThreeTracePocketStart{} to \VThreeTracePocketEnd{}. This is the
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\linewidth]{figures/v3/v3_sweep_repair_gain.pdf}
...
\VThreeSweepCEMRegret{} and combined repair has
\VThreeSweepRepairedRegret{}, for a mean reduction of \VThreeSweepGain{}.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{figures/v3/v3_learned_closed_loop.pdf}
...
\label{fig:v3-learned-closed}
\end{figure}

The learned-ensemble bridge is smaller but important. Vanilla learned \cem{}
has regret \VThreeLearnedCEMRegret{}, while calibrated learned \cem{} has
\VThreeLearnedCalibratedRegret{}, a \VThreeLearnedGain{} improvement. In the
short closed-loop stress test, vanilla \cem{} has return
\VThreeClosedLoopCEM{} and combined repair has \VThreeClosedLoopRepair{}, a
\VThreeClosedLoopGain{} point gain.

\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{figures/v3/v3_tail_gap_vs_regret.pdf}
...
\label{fig:v3-gap-regret}
\end{figure}

Finally, selected tail gap and controlled regret have correlation
\VThreeTailGapRegretCorr{} across the controlled rollout audit. This does not
