...
\section{V3 Evidence Ledger}
\label{app:v3-ledger}

The v3 evidence script adds a paper-facing layer over the existing artifacts.
...
\toprule
V3 artifact & Contents & Reviewer use \\
\midrule
...
\bottomrule
\end{tabular}
\caption{V3 cached evidence artifacts.}
\label{tab:v3-ledger}
\end{table}

The evidence count matters. The v3 script reads \VThreeControlledRows{}
controlled rollout rows, \VThreeTraceRows{} trace rows,
\VThreeSweepRows{} sweep rows, \VThreeRepairGainRows{} paired sweep-gain rows,
and \VThreeLearnedRows{} learned rollout rows. These are small CPU artifacts,
...
This decomposition explains why the v3 audit reports proposal drift and elite
...
\VThreeTraceTailGapStart{} and elite pocket mass is
\VThreeTracePocketStart{}. By the final iteration, selected tail gap is
\VThreeTraceTailGapEnd{} and elite pocket mass is
\VThreeTracePocketEnd{}. This is not only a final-outcome difference. It is an
...
iteration count, the mechanism claim would be fragile. Instead, the v3 sweep
summary reports mean vanilla regret \VThreeSweepCEMRegret{} and repaired
regret \VThreeSweepRepairedRegret{}, a \VThreeSweepGain{} mean improvement.
...
Figure~\ref{fig:v3-sweep} should be read through this lens. The repair is most
...
regret \VThreeCEMRegret{}, while static proposal has
\VThreeStaticRegret{}. CEM is worse by \VThreeCEMMinusStatic{} regret points,
...
controlled repair has regret \VThreeBestRepairRegret{}, far below vanilla
CEM's \VThreeCEMRegret{}. This does not mean the method is production-ready.
...
\VThreeLearnedCEMRegret{}, while calibrated learned CEM has regret
\VThreeLearnedCalibratedRegret{}.
...
step re-enters the same model-error pocket. The v3 closed-loop check is a
small sanity test, not a full answer.

In the short receding-horizon evaluation, vanilla CEM has return
\VThreeClosedLoopCEM{} and combined repair has return
\VThreeClosedLoopRepair{}, an improvement of \VThreeClosedLoopGain{}. This
...
\path{controlled_planner_table.csv}; Figure~\ref{fig:v3-rank}. \\
Selected model optimism is tied to regret. &
\path{controlled_rollout_audit.csv}; Figure~\ref{fig:v3-gap-regret}. \\
CEM refits concentrate on the error pocket across iterations. &
\path{cem_trace_progression.csv}; Figure~\ref{fig:v3-trace}. \\
Combined repair improves sweep regret. &
\path{sweep_summary.csv}; Figure~\ref{fig:v3-sweep}. \\
Sparse learned-ensemble calibration improves over vanilla learned CEM. &
\path{learned_planner_table.csv}; Figure~\ref{fig:v3-learned-closed}. \\
Closed-loop repair improves short-horizon return. &
\path{closed_loop_table.csv}; Figure~\ref{fig:v3-learned-closed}. \\
Benchmark-scale transfer remains unsupported. &
\path{docs/claims.json}. \\
\bottomrule
\end{tabular}
\caption{Claim trace for v3 manuscript statements.}
...
\item The final PDF must not be the old 7-page draft. The v3 build writes a
25-page final artifact.
\item The repository must contain the final PDF. The v3 build writes it under
\path{paper/final/}.
\item The Desktop PDF must match the repo PDF. The v3 audit checks hashes.
\item The source map must point to this folder and repo. The v3 audit checks
that row.
...
The writing policy for v3 is therefore strict. Phrases such as "best of $n$"
...
It is valid to say that vanilla \cem{} has regret \VThreeCEMRegret{} in the
verified run, that static proposal has regret \VThreeStaticRegret{}, that
equal-budget static proposal has regret \VThreeEqualStaticRegret{}, and that
the best repair reduces regret by \VThreeBestRepairGain{} points. It is also
valid to say that selected tail gap and regret are strongly coupled in this
benchmark, with correlation \VThreeTailGapRegretCorr{}. The controlled setting
...
\cem{} reduces regret from \VThreeLearnedCEMRegret{} to
\VThreeLearnedCalibratedRegret{} in this bridge experiment. It does not
...
controlled family: across \VThreeSweepRows{} sweep rows, combined repair
reduces mean regret from \VThreeSweepCEMRegret{} to
\VThreeSweepRepairedRegret{}. The closed-loop test supports the narrower
statement that the repaired planner improves a short receding-horizon return
from \VThreeClosedLoopCEM{} to \VThreeClosedLoopRepair{}. The closed-loop
...
are unsupported in v3. They may be valuable future work, but they are not
...
Table~\ref{tab:experiment-matrix} records the v3 experiment matrix as a
...
the result must be visible in the v3 ranking figure. \\
Trace progression &
...
\bottomrule
\end{tabular}
\caption{V3 experiment matrix. Each row has a different inferential role.}
...
whether the optimizer adapted toward the wrong signal. The v3 trace figure
...
The experiments in v3 are small enough that overclaiming statistical
...
support is the full controlled artifact and the v3 planner ranking, not a
...
The v3 paper's present statistics are best read as deterministic artifacts of
...
checked mechanically. This is more important for v3 than adding decorative
...
The sweep in v3 varies population, iteration count, elite fraction, and seed.
...
The current v3 sweep is enough to show that the main result is not a single
...
proposal would not test the refit loop. Fourth, v3 paper numbers must be
derived from repository artifacts, not typed manually. The macros file written
by \path{experiments/v3_cached_evidence.py} is a practical guardrail: it
...
The experiments are designed to be run without high memory pressure. The v3
...
v3 audit follows that pattern by reusing full CPU artifacts, deriving
...
A reviewer should read the v3 results in a specific order. First, inspect the
...
A third rejection path is duplicate-template suspicion. The v3 paper should
...
A fourth rejection path is insufficient evidence. The v3 response is to be
...
\path{paper/final/} with a v3 filename.
\item The visible Desktop copy has the same SHA-256 hash as the repository
final PDF.
\item The source map row names the v3 Desktop PDF, the local source folder,
...
\item \path{experiments/v3_cached_evidence.py} regenerates the v3 summary,
CSV ledgers, figures, and macros.
\item The v3 summary counts match the manuscript macros.
\item \path{scripts/run_claim_audit.py} passes against
\path{docs/claims.json}.
\item The v3 audit script checks page count, hashes, evidence values, stale
...
because it sees more candidates. The current v3 comparisons are deliberately
...
strongest when these measures move together, as they do in the v3 vanilla
\cem{} trace.
...
collapse around visually plausible but dynamically invalid rollouts. The v3
...
repair that requires an ensemble changes memory and training cost. The v3
...
A fresh agent or reviewer should be able to reproduce the v3 artifact without
guessing which file is authoritative. The source folder is the repository root.
The paper source is \path{paper/iclr_submission/main.tex}. The v3 build script
is \path{scripts/build_v3_paper.py}. The paper-facing evidence cache is
\path{results/v3_cached_evidence/}. The final repository PDF is stored under
\path{paper/final/}. The visible Desktop PDF has the same v3 filename.
...
\path{docs/claims.json}. Third, run the v3 cached evidence script to regenerate
summary values and figures. Fourth, rebuild the paper so the macros and
figures are synchronized. Fifth, run the v3 audit to check page count, hashes,
source-map consistency, and stale artifacts.

This recipe deliberately avoids requiring expensive reruns for every fresh
session. The full CPU artifacts are already committed. The v3 script is a
...
only wants to check paper consistency can regenerate the v3 layer quickly.
...
future version could add denser oracle sweeps, but the v3 conclusion does not
...
to attack. The v3 version instead aims to be hard to reject on internal
...
\section{V4 Benchmark Roadmap}
\label{app:v4-roadmap}

The v3 manuscript is submission-ready as a bounded mechanism paper. The most
valuable v4 would be an external benchmark expansion. The first target should
be a low-dimensional continuous-control environment with learned dynamics and
cheap true evaluation. This would allow candidate-level audit labels for a
subset of planned trajectories. The second target should be a latent-dynamics
benchmark where the planner operates in learned latent space and true returns
are observed after execution. The third target should compare CEM against
...
The roadmap should not be used to weaken v3. A paper can be complete while
still having future work. V3's job is to establish the mechanism cleanly and
make the audit contract reproducible. V4's job would be to test how often the
mechanism matters in larger systems.

\section{Archive Contract}
\label{app:archive}

The final v3 artifact is
\path{best-of-n-MPC-CEM-latent-world-models-v3.pdf}. The build copies this PDF
...
contain the v3 evidence summary, CSV ledgers, figures, macros, claim audit, and
final PDF. If a later version adds benchmark-scale evidence, it should create
a new versioned evidence layer and final PDF rather than overwriting v3 claims
in place.
